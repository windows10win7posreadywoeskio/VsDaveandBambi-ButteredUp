...
class FreeplayState extends MusicBeatState
{
	...
	private var AllPossibleSongs:Array<String> = ["bupe", "mods", "cummingst"];
	...
	override function create()
	{
		...
		// attempted failsafe????
		//CurrentSongIcon = new FlxSprite(0,0).loadGraphic("assets/shared/images/" + "week_icons_" + AllPossibleSongs[CurrentPack].toLowerCase() + ".png");
	...
	{
		songs.push(new SongMetadata(songName, weekNum, songCharacter, 0xFF000000));
	}
    ...
    public function GoToActualFreeplay()
    {
		...
		//WeekData.setDirectoryFromWeek();
		if (AllPossibleSongs[CurrentPack] == "mods" && InMainFreeplayState == true)
			{
				WeekData.setDirectoryFromWeek();
			}
    ...
    public function LoadProperPack()
		{
			switch (AllPossibleSongs[CurrentPack].toLowerCase())
			{
				...
				case 'bupe':
					addWeek(['Handicap'], 1, ['dave']);
					addWeek(['Husk', 'Descrimination'], 1, ['bambi']);
					addWeek(['Decagon', 'Placate', 'Valediction'], 4, ['seri']);
				case 'joke':
				...
				case 'extrasandfanmades':
					...
					addWeek(['WIP'], 69, ['bf']);
				case "":
					addWeek(['keep-going'], 54, ['bf']);
				case "cummingst":
					addWeek(['Handicap', 'Husk', 'Descrimination', 'Valediction'], 420, ['dave']);
			}
		}
